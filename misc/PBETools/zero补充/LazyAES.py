...
function AES_Encryp(str,key) {
  var hash = CryptoJS.AES.encrypt(str, key);
  return hash;
}

function AES_Decryp(str,key) {
  var hash = CryptoJS.AES.decrypt(str, key).toString(CryptoJS.enc.Utf8);
  return hash;
}'''


def AES_Decryp(string,key):
    global jsstr
    res = ""
    try:
        content = js2py.EvalJs()  # 实例化解析js对象
        content.execute(jsstr)  # js转python代码
        res = content.AES_Decryp(string,key)
    except:
        pass
    return res

def AES_Encryp(string,key):
    global jsstr
    res = ""
    try:
        content = js2py.EvalJs()  # 实例化解析js对象
        content.execute(jsstr)  # js转python代码
        res = content.AES_Encryp(string,key)
    except:
        pass
    return res

def AES_Decryp_(string,key):
    res = ""
    ctx = MiniRacer()
    with open('AES.JS', mode='r', encoding='utf-8') as f:
        code = f.read()
    f.close()
    try:
        ctx.eval(code)
        res = ctx.call("AES_Decryp",string,key)
    except:
        pass
    return res

def AES_Encryp_(string,key):
    res = ""
    ctx = MiniRacer()
    with open('AES.JS', mode='r', encoding='utf-8') as f:
        code = f.read()
    ...
    try:
        ctx.eval(code)
        res = ctx.call("AES_Encryp",string,key)
    except:
        pass
    return res
...
x = AES_Decryp("U2FsdGVkX18yNX2nppxF2v0OBXU1JGisr70kGrSAn6U=","vfree")
print(x)